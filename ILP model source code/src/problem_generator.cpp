...
vector<Problem> problemGenerator() {
	...
	constexpr auto pcProb = 0; // fixed later in the code

	/* Virtual probability */
	double virtProb = 0.15;

	/* Parallel task probability */
	double paraProb = 0.5;
			...
			for (int task = 0; task < Tasks[inst]; task++) {
				...
				if (limit < pcProb) {
					...
					if (!object.T[task].para.empty()) {
						...
						for (int idx = 0; idx < R[task].size(); idx++) {
							if (R[task][idx] == 1) {
								tempVector.emplace_back(idx - Agents[inst]);
							}
						}

						sort(tempVector.begin(), tempVector.end());
						set_difference(l.begin(), l.end(), tempVector.begin(), tempVector.end(), inserter(tmpTasksForPrec, tmpTasksForPrec.begin()));
					}
					else {
						tmpTasksForPrec = l;
					}
		...
		for (int task = 0; task < Tasks[inst]; task++) {
			...
			if (object.T[task].virt == 1) {
				...
				if (!object.T[task].para.empty()) {
					...
					sort(tempVector.begin(), tempVector.end());
					set_difference(vPhysicalTasks.begin(), vPhysicalTasks.end(), tempVector.begin(), tempVector.end(), inserter(tmpTasksForPrec, tmpTasksForPrec.begin()));
				}
				else {
